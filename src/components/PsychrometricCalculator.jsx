...
export default function PsychrometricCalculator() {
  ...
  return <div className="psy-app">
    <div className="psy-shell">
      ...
      <section className="psy-panel psy-audit"><div className="psy-panel-heading"><div><span className="psy-eyebrow">04 / CRITIQUE MODE</span><h2>Calculated. Then checked again.</h2></div><div className={`psy-score ${result.audit?.acceptable?'':'psy-score-warning'}`}>{format(result.audit?.score??0,2)}<small> / 10</small></div></div>
        <p>{result.audit?.explanation??'No complete valid state to check. Supply two independent, consistent properties.'}</p><p className="psy-muted">Numerical agreement score = max(0, 10 − 2 × largest error/tolerance). 0–4 unacceptable; 5–7 needs revision; 8–10 acceptable. Decimal scores use 8 as the acceptance threshold. This measures implementation agreement, not measurement accuracy.</p>
        {result.audit&&<details><summary>Inspect all nine property comparisons</summary><div className="psy-table-scroll"><table><thead><tr><th>Property (SI)</th><th>Original</th><th>Independent</th><th>Absolute error</th><th>Tolerance</th></tr></thead><tbody>{result.audit.comparisons.map(row=><tr key={row.key}><th>{row.key} ({unit(row.key)})</th><td>{format(row.original,6)}</td><td>{format(row.recomputed,6)}</td><td>{row.difference.toExponential(2)}</td><td>{row.tolerance}</td></tr>)}</tbody></table></div></details>}
        {result.references&&<details><summary>{result.references.every(r=>r.pass)?'✓':'⚠'} ASHRAE reference checks · {result.references.filter(r=>r.pass).length}/{result.references.length} passed</summary><p className="psy-muted">Fixed regression checkpoints from ASHRAE 2017, chapter 1, Tables 1–3 and Example 1, as transcribed in PsychroLib tests. These validate the equations; arbitrary states are checked by independent recomputation, not claimed as exact table entries.</p><div className="psy-table-scroll"><table><thead><tr><th>Reference</th><th>Calculated</th><th>Tabulated</th><th>Tolerance</th><th>Result</th></tr></thead><tbody>{result.references.map(row=><tr key={row.label}><th>{row.label}</th><td>{format(row.actual,7)}</td><td>{row.expected}</td><td>{row.tolerance.toPrecision(3)}</td><td>{row.pass?'Pass':'FLAG'}</td></tr>)}</tbody></table></div></details>}
      </section>
      <section className="psy-learn"><span className="psy-eyebrow">05 / THE SCIENCE, MADE SIMPLE</span><h2>Meet the properties.</h2><p className="psy-muted">Nine ways to describe the same air. Formulas below use SI units; W is per kg of dry air.</p><div className="psy-lesson-grid">{lessons.map(([k,description,formula])=><article className="psy-panel" key={k}><span className="psy-symbol">{k}</span><h3>{labels[k]}</h3><p>{description}</p><code>{formula}</code></article>)}</div></section>
